...
### 0. Make all software up-to-date
  ...
  > 1. Check Avavilable Package Lists
    `$ sudo apt-get update`
...
### 2. Create a new user `grader` with sudo privileges
   ...
   > `$ sudo adduser grader`  
   >
   > 2. Give Sudo Access to the user `grader`
   > `$ sudo visudo`
   >
...
### 3. Generate SSH key pair 
   ...
   > 1. Generate encrypted key pair in the local machine
   >
   > ```$ ssh-keygen```
   >
   ...
   >
   >3. Place a public key into the `grader`'s home directory
   >
   > ```
...
### 4. Force key-based authentication 
   ...
   > 1. Disable Password Based Login
   >```$ sudo vim /etc/ssh/sshd_config```
   >
   ...
   > 
   > 3. Enable grader user remote ssh login
   > ```$ sudo vim /etc/ssh/sshd_config```
   >
...
### 5. Install Web Server
   ...
   > 1. Get your server responding to HTTP requests
   > ```
   >    $ sudo apt-get install apache2
   > ```
   >   
   > 2. Configure Apache to hand-off certain requests to an application handler
   > ```
...
### 6. Set up Database Server
   ...
   >   $ psql
   >```
   >3. Create a new user `catalog` with right to create database
   >```
   >   CREATE ROLE catalog WITH LOGIN;
   >   ALTER ROLE catalog CREATEDB;
   >```
   >4. Set password for user `catalog`
   >```
   >   \password catalog
   >   \q
   >   exit
   >```
   >5. Create a new Linux user `catalog` with a new database
   >```
   >   $ sudo adduser catalog
   >   $ sudo visudo
   >```
   >add a line `catalog ALL=(ALL:ALL) ALL` after the `root` line<br>
   >create a new database (`catalog` user) `$ createdb catalog`
   >
   >6. switch the database from SQLite to PostgresSQL
...
### 7. Clone a catalog app from github
   ...
   >```
   >2. Clone the web app to the server's `catalog` folder
   > ```
   >   $ sudo cd /var/www/catalog
   >   $ sudo git clone https://github.com/ayrka39/FSDN-4Catalog.git catalog
   >```
   >3. Modify client_secrets.json file for google authentication
...
### 9. Get a flask app work
   ...
   >1. create a .wsgi file in /var/www/catalog directory
   >```
...
### 10. Populate the database
   ...
   >1. Activate the virtualenv in /var/www/catalog/catalog directory
   >```
   >   $ . venv/bin/activate
